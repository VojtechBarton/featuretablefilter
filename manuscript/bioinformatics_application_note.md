...
`featuretablefilter` is implemented in R (≥4.5.0) and integrates with the Bioconductor ecosystem. The package imports core infrastructure including `ggplot2` for visualization, `S4Vectors` for tabular data handling and `zoo` for smoothing utilities, while `phyloseq`, `TreeSummarizedExperiment`, `vegan`, `ComplexHeatmap`, `shiny`, `DT` and other packages are listed as suggested dependencies for extended functionality.
...
1. **Atomic filtering functions** implement individual filtering strategies:
   ...
   * `filter_cross_talk()` and `filter_index_hopping()` detect and mitigate index-hopping leakage.
...
3. **The integrated pipeline** `run_filtering_pipeline()` orchestrates loading, filtering, QC, visualization and reporting in one call. Inputs can be `data.frame`, `matrix`, TSV/CSV files, `phyloseq` or `TreeSummarizedExperiment` objects; outputs preserve the original class, including metadata and phylogenetic trees. The pipeline optionally produces a plain-text, Markdown or PDF report, coverage and abundance plots, presence–frequency analyses, scree/sensitivity analyses and a Shiny dashboard (`runDashboard()`).
...
All functions are documented with `roxygen2` and unit-tested with `testthat`, and functionality is illustrated in two package vignettes: an introduction (“featuretablefilter: Feature Table Filtering for Microbiome Data”) and a complete end-to-end tutorial. The source repository provides a Dockerfile and docker-compose configuration for reproducible deployment, as well as GitHub Actions workflows for continuous integration across platforms.
...
* Source code, documentation and issue tracker: https://github.com/VojtechBarton/featuretablefilter
* Bioconductor: submission in progress (https://bioconductor.org/packages/featuretablefilter)
...
3. Huber, W., Carey, V. J., Gentleman, R., Anders, S., Carlson, M., Carvalho, B. S., Bravo, H. C., Davis, S., Gatto, L., Girke, T., Gottardo, R., Hahne, F., Hansen, K. D., Irizarry, R. A., Lawrence, M., Love, M. I., MacDonald, J., Obenchain, V., Olés, A. K., Pagès, H., Reyes, A., Shannon, P., Smyth, G. K., Tenenbaum, D., Waldron, L., & Morgan, M. (2015). Orchestrating high-throughput genomic analysis with Bioconductor. *Nature Methods*, 12(2), 115–121.

4. Huang, R., Soneson, C., Ernst, F. G. M., Rue-Albrecht, K. C., Yu, G., Hicks, S. C., & Robinson, M. D. (2021). TreeSummarizedExperiment: a S4 class for data with hierarchical structure. *F1000Research*, 9, 1246.

5. Nearing, J. T., Douglas, G. M., Comeau, A. M., & Langille, M. G. I. (2022). Denoising the effects of sequencing depth and conditioning on the analysis of microbiome data. *PeerJ*, 10, e13061.

6. Stewart, C. J. (2019). Microbiome studies need to adopt a more robust experimental design. *Microbiome*, 7, 105.

7. Weiss, S., Xu, Z. Z., Peddada, S., Amir, A., Bittinger, K., Gonzalez, A., Lozupone, C., Zaneveld, J. R., Vázquez-Baeza, Y., Birmingham, A., Hyde, E. R., & Knight, R. (2017). Normalization and microbial differential abundance strategies depend upon data characteristics. *Microbiome*, 5, 27.
